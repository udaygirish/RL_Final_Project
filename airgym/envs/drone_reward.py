...
def compute_reward(startState, goalState, currentState):    
    ...
    if currentState["collision"]:
        ...
        # dist_to_goal_x = curr[0] - goalState[0]
        # dist_to_goal_y = curr[1] - goalState[1]
        # dist_to_goal_z = curr[2] - goalState[2]
        ...
        closest_points = find_closest_point_kdtree(curr, points, goalState, 20)
        ...
        if closest_points is not None:
            ...
            print("Distance to Goal Closest", dist_to_goal_closest)
            print("Reward from KDTree Added")
        ...
        ############ Total Reward ###########        
        reward = 5 * reward_dist + 1.5*reward_angle + reward_kdtree + reward_speed - 0.01 * smoothness_penalty
        #####################################

    done = 0
    # Terminate if the quadrotor is within a certain radius of the goal
    dist_to_goal = np.linalg.norm(curr - goalState)
    if dist_to_goal < 12 or reward <= -10:
        if dist_to_goal < 12:
            reward = reward + 30
            print("Reached Goal")
        done = 1

    # Hard Level Constraints - Out of Bound 
    if abs(curr[0]) > 150 or abs(curr[1]) > 100 or abs(curr[2]) > 100:
        reward = -100
        done = 1
